...
# Integrate the motion equation to find the angular displacement
theta = np.cumsum(equation_of_motion * (t_eval[1] - t_eval[0]))

# Plot the angular displacement
plt.plot(t_eval, theta, label='Angular Displacement (theta)')
plt.xlabel('Time (s)')
plt.ylabel('Angular Displacement (rad)')
plt.title('Angular Displacement of the Spring-Mass System')
plt.legend()
plt.grid()
plt.show()
